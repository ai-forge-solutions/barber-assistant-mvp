...
1. `Arranque` — 29€/mes mes a mes. Sirve como precio ancla y opción flexible.
2. `Barbería anual` — recomendado: 15€/mes con compromiso anual. Se presenta como ahorro del 48% frente al mes a mes y como “menos que un corte al mes”.
3. `Barbería pro` — 39€/mes para equipos/locales con más sillas. Hace que el recomendado parezca razonable, no “barato”.

El plan recomendado se integra también en la home porque la landing ya está enfocada a conversión. `/pricing` queda como página específica para quien quiera comparar planes o resolver objeciones antes de poner tarjeta.

## Tarjeta desde el primer paso

El checkout de `/api/billing/checkout` usa Stripe Checkout en modo `subscription` con:

- `payment_method_collection: 'always'` para pedir método de pago aunque el precio/trial inicial hiciera que hoy no se cobre.
...
- dominio verificado para Apple Pay / Google Pay.

## Google Pay

No requiere UI propia si usamos Stripe Checkout. Dificultad de código: baja. Stripe muestra Google Pay automáticamente cuando:

1. está habilitado en Stripe;
2. el dominio está verificado;
3. el navegador/dispositivo del comprador lo soporta;
4. el price/currency permite ese método.

La parte pendiente no es de React/Next, sino de setup en la cuenta Stripe y dominio real.
...
1. confirmar si “15€/mes anual” significa:
   - factura mensual con compromiso anual, o
   - factura anual de 180€ cobrada de una vez;
2. claves live/test de Stripe;
3. IDs reales de los prices;
4. webhook secret;
5. dominio definitivo para verificar Google Pay;
6. política de trial/oferta inicial: cuántos días gratis y cuándo se hace el primer cargo;
7. si se debe bloquear dashboard inmediatamente cuando `past_due` o dar periodo de gracia.
